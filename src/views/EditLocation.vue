...
export default {
  ...
  computed: {
    location () {
      ...
      return this.$store.getters.allLocations.find(item => item.value === id)
    },
    oldlocation () {
      const id = parseInt(this.$route.params.id)
      return this.$store.getters.allLocations.find(item => item.value === id)
  ...
  methods: {
    editLocation () {
      ...
      locationData.append('name', this.location.label)
      locationData.append('description', this.location.description)
      this.$store.dispatch('editLocation', locationData)
    }
  }
}
</script>
